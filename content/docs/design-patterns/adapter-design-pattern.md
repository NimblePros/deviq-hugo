...
params:
  image: /docs/design-patterns/images/adapter-design-pattern.png
---
...
Notice that the `INotificationAdapter` abstraction does not expose any details about how the notification might be sent. An Adapter should ideally be written using the abstractions of the application that will be using it, and should avoid exposing implementation details implicitly or explicitly. Recall that the [Dependency Inversion Principle](/docs/principles/dependency-inversion-principle/) requires that abstractions should not depend on details.

The Adapter pattern also enables the [Open-Closed Principle](/docs/principles/open-closed-principle/), since new functionality can be added to the system's use of notifications without requiring existing code to be modified. Instead, new instances of the `INotificationAdapter` can be added, extending the ways in which notifications may be sent.

Adapters are frequently used in [Domain-Driven Design](/docs/domain-driven-design/ddd-overview/) as part of [Anti-Corruption Layers](/docs/domain-driven-design/anti-corruption-layer/).
