...
A better approach than the Singleton pattern is to follow the [Explicit Dependencies Principle](/docs/principles/explicit-dependencies-principle/) from for classes that depend on a singleton type. Use [dependency injection](/docs/practices/dependency-injection/) to pass the object into the type that needs it, and configure the services/IOC container to enforce the object's singleton lifetime behavior.
